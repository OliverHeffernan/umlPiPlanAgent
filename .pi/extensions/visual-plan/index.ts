...
export default function visualPlanExtension(pi: ExtensionAPI): void {
  ...
  pi.on("before_agent_start", (event) => {
    if (!enabled) return;
    const instructions = `[VISUAL PLANNING MODE]
You are planning, not implementing. Do not mutate project code or configuration. Read and investigate the project, ask focused questions, and maintain the plan with ${TOOL_NAME}.
