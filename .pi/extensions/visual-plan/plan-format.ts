...
export function validatePlan(markdown: string, phase: PlanPhase): string[] {
  ...
  if (Buffer.byteLength(markdown, "utf8") > 200_000) errors.push("The plan exceeds 200 KB.");
  if (!/^#\s+\S/m.test(markdown)) errors.push("The plan needs a level-one title.");

  if (phase === "idea" && ANY_MERMAID_FENCE.test(markdown)) {
    errors.push("Idea exploration must not contain diagrams yet. Move to implementation phase first.");
  }
  if (phase === "implementation" && !UML_FENCE.test(markdown)) {
    errors.push("Implementation plans require a Mermaid classDiagram or stateDiagram-v2 block.");
  }
  return errors;
}
